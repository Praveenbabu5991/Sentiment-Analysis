...
def declaration_variables(Xt,Yt,Xte,Yte,index,cell_input):
    ...
    with tf.variable_scope("RNN"):
        ...
        tf.add_to_collection("vars",cell)
...
def training(Xt, Yt, Xte, Yte, index):
    ...
    if index == 0:
        variable_list = declaration_variables(Xt,Yt,Xte,Yte,index,cell_input=None)
        ...
        if index == 1:
            cell = graph.get_collection("cell")
        else:
            cell = new_graph.get_collection("cell")
            #type checking needed
        with new_graph.as_default():
            variable_list = declaration_variables(Xt, Yt, Xte, Yte, index, cell)
            X, Y, loss, train, batch_, label_, prediction, test_batch_, test_label_, accuracy = variable_list
